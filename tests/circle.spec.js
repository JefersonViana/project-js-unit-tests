...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    expect(circle('deu bom')).toBeUndefined(undefined);
    // Teste se circle retorna um objeto.
    const tipo = circle(2);
    expect(typeof tipo).toEqual('object');
    // Teste se o objeto retornado possui 3 propriedades.
    const objetos = Object.keys(tipo);
    ...
    expect(tipo.circumference).toBe(12.56);
    // Teste se dentro do objeto retornado, a função retorna uma `key` com `value` igual à área correta para um círculo de raio 3.
    const area = circle(3); 
    expect(area.area).toBeCloseTo(28.26);
    // Teste se a função retorna, em um objeto, os dados corretos de um círculo de raio 3.
    let teste1 = area.radius;
    let teste2 = area.area;
    let teste3 = area.circumference;
    expect(teste1).toBe(3);
    expect(teste2).toBeCloseTo(28.26);
    expect(teste3).toBeCloseTo(18.84);
  });
});
